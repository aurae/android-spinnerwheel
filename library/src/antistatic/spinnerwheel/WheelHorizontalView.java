...
public class WheelHorizontalView extends AbstractWheelView {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (widthMode == MeasureSpec.EXACTLY) {
			...
			width = Math.max(getItemDimension() * (mVisibleItems - mItemOffsetPercent / 100), getSuggestedMinimumWidth());
	...
	private int calculateLayoutHeight(int heightSize, int mode) {
		...
		// MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED),
				MeasureSpec.makeMeasureSpec(400, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(height - 2 * mItemsPadding, MeasureSpec.EXACTLY));
	...
	protected void drawItems(Canvas canvas) {
		...
		int left = (mCurrentItemIdx - mFirstItemIdx) * iw + (iw - getWidth()) / 2 - (mItemMargins / 2);
