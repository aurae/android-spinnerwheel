...
public abstract class AbstractWheel extends View {
	...
	protected void initData(Context context) {
		...
		mScroller = createScroller(new WheelScroller.ScrollingListener() {
			...
			public void onTouchUp() {
				if (!mIsScrollingPerformed) onScrollTouchedUp(); // if scrolling IS performed, we
	...
	public void scroll(int itemsToScroll, int time) {
		...
		onScrollTouched(); // we have to emulate touch when scrolling spinnerwheel programmatically
							// to light up stuff
	...
	/**
	 * Sets the velocity factor for the scroller. Be wary of setting 0.0f here, as it hinders
	 * scrolling altogether.
	 * 
	 * @param factor
	 *            the scrolling factor to set
	 */
	...
	private ItemsRange getItemsRange() {
		...
		if (!isCyclic()) {
			if (start < 0) start = 0;
			if (end > mViewAdapter.getItemsCount()) end = mViewAdapter.getItemsCount();
		}
	...
	static int supportGetToolType(MotionEvent event, int pointerIndex) {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
			// For devices above ICS, use the tool type method directly
			return event.getToolType(pointerIndex);

		} else {
			// Below ICS, check the MotionEvent's meta state
			int metaState = event.getMetaState();

			// MetaState == 512 refers to MotionEvent.TOOL_TYPE_STYLUS (2)
			if (metaState == 512) return 2;

			// MetaState == 1024 refers to MotionEvent.TOOL_TYPE_ERASER (4)
			if (metaState == 1024) return 4;

			// Else, assume that the finger is used; MotionEvent.TOOL_TYPE_FINGER (1)
			return 1;
		}
	}
}
